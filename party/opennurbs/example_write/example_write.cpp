  ...
  )
{
  ...
  model.m_sStartSectionComments = "This was file created by OpenNURBS toolkit nurbstools code.";
  ...
  if (bHaveFunctionName && bHaveFileName)
  {
    model.m_properties.m_Notes.m_notes
      = ON_wString::FormatToString(
        L"This .3dm file was made with the OpenNURBS toolkit nurbstools function %s() defined in source code file %ls.",
...
{
  // nurbstools demonstrates how to write a singe points and point clouds
  ONX_Model model;
...
{
  // nurbstools demonstrates how to write a NURBS curve, line, and circle
  ONX_Model model;
  INTERNAL_INITIALIZE_MODEL(model);
...
{
  // nurbstools demonstrates how to write a NURBS surface
  ONX_Model model;
  INTERNAL_INITIALIZE_MODEL(model);
...
{
  // nurbstools demonstrates how to create and write a mesh
  ONX_Model model;
  INTERNAL_INITIALIZE_MODEL(model);
...
static bool write_mesh_with_material_example( const wchar_t* filename, ON_TextLog& error_log )
{
  ...
  // nurbstools demonstrates how to create and write a mesh that uses
...
int main ()
{
  ...
  // this nurbstools paying close attention to write_trimmed_surface_example(),
