...
static const wchar_t* tagUniCharDec          = L"u";      // UNOCODE UTF-16 encoded value as a signed short (0x8...) will be -....
                                                          ...
                                                          // For nurbstools, the single UNICODE code point
...
void ON_TextBuilder::UniDecimal(const wchar_t* value)
{
  ...
  for (;;)
  {
    // For nurbstools, the single UNICODE code point ON_UnicodeCodePoint::Wastebasket U+1F5D1 (decimal 128465)
    //
    // As of March 2018:
  ...
  if (nullptr != run_code_points && 0 != run_code_points[0])
  {
    for (int ci = 0; 0 != run_code_points[ci]; ci++)
    {
      ...
      if (code_point > 0x80 || 1 != utf16_count || code_point != (ON__UINT32)utf16[0])
      {
        ...
        // An nurbstools is the Euro sign (Windows-1252 0x80 = Euro sign, UNICODE U+0080 = xxx control,
        ...
        // For nurbstools, UNOCODE code point U+1F5D1 has UTF-16 encodeing (0xD83D, 0xDDD1)
